...
Phases 1–4 were implemented in PRs #3–#7 and audited in the post-implementation audit (70/70 tests green at audit time; 72/72 as of the follow-up review). Fully-completed spec files have been **removed** — their content lives in git history (`git log -- specs/`) and their outcomes in the audit's scorecard. The v1.3.0 release execution completed on 2026-07-13: the package is published, all release tags have GitHub releases, and the pinned clean-machine command was verified. What remains here is only actionable work:
...
Removed (done — see the [audit](../docs/reviews/2026-07-10-post-implementation-audit.md) §1 and [follow-up review](../docs/reviews/2026-07-13-follow-up-review.md)): 1.1 coverage metric · 1.2 report correctness · 1.3 release execution · 2.1 dogfood · 2.2 example + README · 3.1 structured trace · 3.2 generated importers · 4.1 UX polish · 4.2 OSS hygiene · 5.1 correctness batch (merged in PRs #9/#10; shipped in v1.4.0). The U1 stale-flag time bomb found in the audit was fixed directly (report `--now`), not spec'd. The CI validate-job snippet lockstep gap (template vs. `references/ci-wiring.md`) called out in the follow-up review was also fixed directly, not spec'd — shipped in v1.4.0.

Future candidates (not yet spec'd, in ROI order — see the [audit](../docs/reviews/2026-07-10-post-implementation-audit.md) §5 and the [follow-up review](../docs/reviews/2026-07-13-follow-up-review.md) §5): `doctor` command (one-shot "is collection actually working", closes the silent-config failure mode) · PR trace-lint GitHub Action · hosted demo dashboard · org-export seam.
